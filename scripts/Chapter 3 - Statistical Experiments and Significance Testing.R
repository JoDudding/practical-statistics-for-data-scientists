...
click_rate <-  read_csv('data/click_rates.csv') |>
  clean_names()

imanishi <-  read_csv('data/imanishi_data.csv') |>
  clean_names() |>
...
session_times |>
  ...
  geom_boxplot() +
  labs(x = NULL, y = 'Time (in seconds)')

session_times |>
  group_by(page) |>
  summarise(mean_time = mean(time)) |>
  spread(key = page, value = mean_time) |>
  clean_names() |>
  mutate(page_diff = page_b - page_a) |>
  print()
...
graph <- ggplot(four_sessions, aes(page, time)) +
  geom_boxplot() +
...
summary(anova(lm(time ~ page, data = four_sessions)))
...
chi <- tibble(
  df = rep(c(1, 2, 5, 20), rep(100, 4)),
  x = rep(seq(1, 30, length=100), 4),
  p = dchisq(x, df)
) |>
  mutate(df = factor(df))
...
chi |>
  ...
  annotate('text', x =  25, y = 0.06, label = 'df = 20', size = 2.5) +
  annotate('text', x =   8, y = 0.1 , label = 'df = 5' , size = 2.5) +
  annotate('text', x =   4, y = 0.2 , label = 'df = 2' , size = 2.5) +
  annotate('text', x = 1.5, y = 0.05, label = 'df = 1' , size = 2.5) +
